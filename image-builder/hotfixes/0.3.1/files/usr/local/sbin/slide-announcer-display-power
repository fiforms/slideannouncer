...
trigger (the remote's power key via power-button-monitor.py; in future, a
systemd timer for a schedule, or a menu button in the local web UI backed
by a oneshot slide-announcer-*.service the backend starts) just calls this
CLI rather than each re-implementing the toggle.
