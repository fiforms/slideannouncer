...
#
# Also re-applies the persisted volume/mute level (/data/status/audio-volume,
# /data/status/audio-muted — written by volume-key-monitor.py, the remote's
# volume/mute keys) right after selecting the sink, since a fresh boot or an
# output switch would otherwise leave the new default sink at PipeWire's own
# default (observed to be a flat 40%, not whatever the room was last set to).
#
...
# PipeWire/WirePlumber run under the XDG_RUNTIME_DIR kiosk-start.sh
# exports before backgrounding them there — when that script is the
# caller, this is a no-op (already set, ":-" only fills in when unset).
# But the OTHER caller, system_control.apply_audio_output(), runs inside
# slide-announcer-backend.service, which has no systemd --user session
# (no PAMName=) to inherit XDG_RUNTIME_DIR from automatically — without
# this, every wpctl call below silently talks to no PipeWire instance at
# all: `set-default` "succeeds" (nothing checks its exit code — see above)
# but doesn't actually move where audio goes. This is why the Settings >
# Audio Output HDMI/Headphone Jack switch changed the on-screen selection
# but never actually re-routed sound.
...
# Re-apply the persisted volume/mute level against whichever sink just
# became the default — @DEFAULT_AUDIO_SINK@ always resolves to it, so this
# doesn't need SINK_ID itself.
VOLUME_FILE="/data/status/audio-volume"
MUTED_FILE="/data/status/audio-muted"
VOLUME="100"
if [ -f "$VOLUME_FILE" ]; then
	candidate="$(cat "$VOLUME_FILE" 2>/dev/null | tr -d '[:space:]')"
...
MUTED="0"
if [ -f "$MUTED_FILE" ] && [ "$(cat "$MUTED_FILE" 2>/dev/null | tr -d '[:space:]')" = "true" ]; then
	MUTED="1"
fi
