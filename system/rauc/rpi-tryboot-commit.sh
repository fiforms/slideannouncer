...
# "commits" it: flips config.txt's permanent os_prefix= to point at the
# new slot, tells RAUC the update is good, then immediately reboots
...
sed -i -E "s#^os_prefix=slot[AB]/#os_prefix=slot${LETTER}/#" "${BOOTFW}/config.txt"
